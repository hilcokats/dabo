...
public interface Opponent extends Player {
	...
	default boolean draws(Card card) {
		return draws(card.getName());
	}

	default boolean plays(Card card) {
		return plays(card.getName());
	}

	default boolean discards(Card card) {
		return discards(card.getName());
	}

	default boolean removesFromGame(Card card) {
		return removesFromGame(card.getName());
	}

	default boolean commands(Card card) {
		return commands(card.getName());
	}
	
	default boolean owns(Card card) {
